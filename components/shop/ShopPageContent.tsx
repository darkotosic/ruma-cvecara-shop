...
import { formatLocationName } from "@/lib/seo";
...
export default function ShopPageContent() {
  ...
  useEffect(() => {
    if (currentSlice.length > 0) {
      trackViewItemList(`Shop-${location.slug}`, currentSlice);
    }
  }, [currentSlice, location.slug]);
      ...
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="rounded-full bg-white px-4 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-primary-dark/70 shadow-inner shadow-primary/10">
            Dostava za grad: {location.naziv ?? formatLocationName(location)}
          </div>
        </div>
